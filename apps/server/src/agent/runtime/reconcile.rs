...
pub(in crate::agent) fn start_codex_reconciler(
    session: &Arc<Session>,
...
pub(in crate::agent) fn start_history_reconciler(
    session: &Arc<Session>,
    app_state: Arc<AppState>,
    baseline: HashSet<String>,
) {
    ...
    tokio::spawn(async move {
        loop {
            ...
            if let Some(id) = id
                ...
                && app_state.contains_session(&session)
                && !session.is_deleting()
                && session.compare_and_update_upstream_session_id(None, id)
            {
                return;
            }
...
pub(in crate::agent) fn start_fork_reconciler(session: &Arc<Session>, app_state: Arc<AppState>) {
    let weak = Arc::downgrade(session);
    tokio::spawn(async move {
        loop {
            ...
            if let Ok(Some(id)) = successor {
                ...
                if let Some(session) = weak.upgrade()
                    ...
                    && !session.is_deleting()
                {
                    session.compare_and_update_upstream_session_id(Some(&current_id), id);
                }
            }
        }
    });
