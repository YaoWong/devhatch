...
const newButtonClass = `${legacyButtonFocus} mini-action tw:h-7 tw:gap-[5px] tw:rounded-lg tw:border-input tw:bg-card tw:px-2 tw:py-0 tw:text-[9px] tw:leading-[normal] tw:font-semibold tw:text-inherit tw:transition-none tw:hover:bg-card! tw:hover:text-inherit! tw:focus-visible:border-input! tw:disabled:pointer-events-auto tw:disabled:cursor-default tw:disabled:opacity-[0.42] tw:dark:bg-card! tw:dark:hover:bg-card! tw:[&_svg]:size-3`;
const workspaceSelectClass = "workspace-select tw:flex tw:min-h-[42px] tw:min-w-0 tw:flex-1 tw:cursor-pointer tw:items-center tw:justify-start tw:gap-3 tw:rounded-lg tw:border-0 tw:bg-transparent tw:px-[7px] tw:py-1 tw:text-base tw:leading-[normal] tw:font-normal tw:whitespace-normal tw:text-inherit tw:text-left tw:touch-manipulation tw:[&>span]:min-w-0 tw:[&>span]:flex-1 tw:[&_small]:mt-[3px] tw:[&_small]:block tw:[&_small]:overflow-hidden tw:[&_small]:font-['SF_Mono',monospace] tw:[&_small]:text-[9px] tw:[&_small]:leading-[1.3] tw:[&_small]:font-normal tw:[&_small]:text-[var(--color-text-faint)] tw:[&_small]:text-ellipsis tw:[&_small]:whitespace-nowrap tw:[&_strong]:block tw:[&_strong]:overflow-hidden tw:[&_strong]:text-[13px] tw:[&_strong]:font-bold tw:[&_strong]:text-ellipsis tw:[&_strong]:whitespace-nowrap";
      ...
      <div className="workspace-list">
        {workspaces.length ? workspaces.map((workspace, index) => {
          ...
          return (
            <div
              key={workspace.id}
              className={`workspace-item tw:group/workspace tw:flex tw:min-h-[52px] tw:w-full tw:items-center tw:gap-1 tw:rounded-xl tw:border tw:p-1 ${selected ? "active tw:border-input tw:bg-background tw:[&:hover]:border-input" : "tw:border-transparent tw:bg-transparent tw:[&:hover]:border-border"}`}
            >
              {renaming ? (
