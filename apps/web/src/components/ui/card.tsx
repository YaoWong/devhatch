...
function CardHeader({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "tw:group/card-header tw:@container/card-header tw:grid tw:auto-rows-min tw:items-start tw:gap-1 tw:rounded-t-xl tw:px-(--card-spacing) tw:has-data-[slot=card-action]:grid-cols-[1fr_auto] tw:has-data-[slot=card-description]:grid-rows-[auto_auto]",
        className
      )}
      {...props}
    />
