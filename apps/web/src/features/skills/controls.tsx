    ...
    <div className="tw:flex tw:w-fit tw:overflow-hidden tw:rounded-[9px] tw:border tw:border-border tw:bg-[var(--color-surface-muted)] tw:skills-max-480:w-full" role="group" aria-label="Filter by source">
      {(["all", "custom", "repository"] as const).map((option) => (
        <Button
          type="button"
          variant="ghost"
          className={`tw:h-10 tw:min-w-0 tw:flex-1 tw:rounded-none tw:px-2.5 tw:text-xs tw:font-medium tw:transition-colors tw:duration-150 tw:first:rounded-l-lg tw:last:rounded-r-lg tw:[@media(pointer:coarse)]:h-11 ${value === option ? "tw:bg-card tw:text-foreground tw:shadow-sm" : "tw:text-muted-foreground"}`}
