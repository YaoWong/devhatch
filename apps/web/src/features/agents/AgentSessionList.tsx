import { Ellipsis, Play, Search, Trash2 } from "lucide-react";
...
export function AgentSessionList({
  ...
  const confirmHistoryResume = (history: HistorySession, presence: string) => {
  ...
  const deleteSession = (live: AgentSession | undefined, history: HistorySession | undefined) => {
    if (live) {
      onDeleteLive(live);
    } else if (history) {
      onConfirm({
  ...
  return (
    <div ref={portalOwnerRef} id={portalOwnerId} className={`${railMenuSectionClass} sessions-section`}>
        ...
        {shouldShowAgentSessionSearch(sessionCount, historyCount, search) && (
          <label className="tw:flex tw:h-10 tw:items-center tw:gap-[5px] tw:rounded-lg tw:border tw:border-border tw:bg-[color-mix(in_srgb,var(--color-surface)_72%,transparent)] tw:px-2 tw:shadow-[0_5px_14px_rgb(29_29_31/4%)] tw:backdrop-blur-[8px] tw:has-[:focus-visible]:border-[var(--color-accent)] tw:has-[:focus-visible]:shadow-[0_0_0_3px_color-mix(in_srgb,var(--color-accent)_16%,transparent)] tw:[@media(pointer:coarse)]:h-11 tw:[&>svg]:w-3 tw:[&>svg]:text-[var(--color-text-faint)]">
            ...
            <Input
              ...
              placeholder="Search"
              value={search}
              className="tw:h-full tw:min-w-0 tw:w-full tw:border-0 tw:bg-transparent tw:text-[calc(11px*var(--app-font-scale))] tw:leading-[1.2]"
              ...
              >
                ...
                <span className={`${sessionActionsClass} ${live ? "session-actions-live" : "session-actions-history"}`}>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon-xs"
                    className={`session-direct-action ${deleteButtonClass}`}
                    aria-label={`Delete ${live?.name ?? history?.title ?? "session"}`}
                    title="Delete session"
                    onClick={() => deleteSession(live, history)}
                  >
                    <Trash2 />
                  </Button>
                  <DropdownMenu
                  ...
                  >
                    <DropdownMenuTrigger
                      ...
                      aria-label={`Session actions for ${live?.name ?? history?.title ?? "session"}`}
                      render={<Button type="button" variant="ghost" size="icon-xs" className={`session-overflow-action ${deleteButtonClass}`} />}
                    >
                      <Ellipsis />
                    </DropdownMenuTrigger>
                    <DropdownMenuContent portalOwner={portalOwnerId} align="end" side="bottom" sideOffset={6} className="tw:w-44">
                      ...
                      <DropdownMenuItem
                        ...
                        onClick={() => {
                          menuTriggerRef.current?.focus();
                          queueMicrotask(() => deleteSession(live, history));
                        }}
                      >
                        <Trash2 />
                        Delete session
