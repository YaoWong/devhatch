...
export function TerminalLayoutPresetControl({ count, value, onChange }: {
  ...
  return <div className="tw:ml-auto tw:flex tw:max-w-full tw:flex-none tw:flex-nowrap tw:gap-0.5" role="group" aria-label={`${count} terminal layout`}>
    {terminalLayoutPresets(count).map((preset) => <Button variant="ghost" size="icon" className="tw:size-10 tw:rounded-lg tw:border-0 tw:bg-transparent tw:text-muted-foreground tw:transition-none tw:hover:bg-muted! tw:aria-pressed:bg-muted tw:aria-pressed:text-foreground tw:aria-pressed:hover:bg-muted! tw:aria-pressed:hover:text-foreground! tw:active:not-aria-[haspopup]:translate-y-0! tw:[@media(pointer:coarse)]:size-11" key={preset} type="button" title={terminalLayoutLabel(preset)} aria-label={terminalLayoutLabel(preset)} aria-pressed={value === preset} onClick={() => onChange(preset)}>{icon(preset)}</Button>)}
  </div>;
}
